    ...
    <div class="card">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item>
          ...
          <el-button link type="primary" @click="toLogin">前往登录</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
const sendCode = async () => {
  ...
  try {
    const { data } = await request.post('/auth/sendVerificationCode', { email: form.email })
    if (data.code === 200) {
...
const onSubmit = () => {
  formRef.value.validate(async valid => {
    ...
    try {
      axios.post(`${config.api}/users/regis`,{...form}).then()
      if (data.code === 200) {
